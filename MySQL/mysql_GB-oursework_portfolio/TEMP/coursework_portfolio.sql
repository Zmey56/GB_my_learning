...
CREATE TABLE currency_used(
	id SERIAL PRIMARY KEY,
	country_id INT UNSIGNED COMMENT 'Ключ на другую таблицу',
...
CREATE TABLE price(
	id SERIAL PRIMARY KEY,
	item_id INT UNSIGNED COMMENT 'Ссылается на валюту, используемую в качестве базовой валюты для данного товара',
	currency_id INT UNSIGNED COMMENT 'Ссылается на валюту, используемую в качестве базовой валюты для данного товара',
...
CREATE TABLE currency_items(
	...
	name VARCHAR(255) NOT NULL UNIQUE COMMENT 'Полное имя',
	is_active BOOL DEFAULT FALSE COMMENT 'Доступен ли этот товар для торговли или нет',
	is_base_currency BOOL DEFAULT FALSE COMMENT 'Торгуется ли товар в основной валюте'
	) COMMENT = 'Связь товара и валюты'
...
CREATE TABLE trader (
    ...
    country_id INT UNSIGNED COMMENT 'Страна, в которой живет.',
    preffered_currency_id INT UNSIGNED COMMENT 'Валюта, которую трейдер предпочитает'
) COMMENT 'юзеры';	
...
CREATE TABLE current_inventory (
	id SERIAL PRIMARY KEY,, 
	trader_id INT UNSIGNED COMMENT 'Ссылка на трейдера',
	item_id INT UNSIGNED COMMENT 'Ссылка на товар',
...
CREATE TABLE trade (
	id SERIAL PRIMARY KEY,
	item_id INT UNSIGNED COMMENT 'Ссылка на товар',	
	seller_id INT UNSIGNED DEFAULT NULL COMMENT 'Ссылка на трейдера',
	buyer_id INT UNSIGNED COMMENT 'Ссылка на трейдера',
	quantity DECIMAL(16,6) DEFAULT NULL COMMENT 'Количество товаров',
	...
	offer_id INT UNSIGNED COMMENT 'Индификатор сделки'
) COMMENT 'Сделки';
...
CREATE TABLE offer (
	id SERIAL PRIMARY KEY,
	item_id INT UNSIGNED COMMENT 'Ссылка на товар',	
	trader_id INT UNSIGNED DEFAULT NULL COMMENT 'Ссылка на трейдера',
	quantity DECIMAL(16,6) DEFAULT NULL COMMENT 'Количество товаров',
